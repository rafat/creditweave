...
var getChainSelector = (config) => {
  const selectorName = config.chainSelectorName ?? "ethereum-testnet-sepolia";
  const selector = ClientCapability.SUPPORTED_CHAIN_SELECTORS[selectorName];
  if (!selector) {
...
var logAudit = (runtime2, event, data) => {
  runtime2.log(`[AUDIT] ${JSON.stringify({
    ts: runtime2.now().toISOString(),
    event,
    ...data
  })}`);
};
...
var fetchJsonWithConfidentialHttp = (runtime2, url, headers) => {
  const rawBody = runtime2.runInNodeMode((nodeRuntime, ...args) => {
    ...
    const response = confidentialHttpClient.sendRequest(nodeRuntime, {
      request: {
        url: requestUrl,
        method: "GET",
        multiHeaders: Object.fromEntries(Object.entries(requestHeaders).map(([key, value2]) => [key, { values: [value2] }]))
      }
    }).result();
  ...
  }, consensusIdenticalAggregation())(url, headers).result();
  return decodeJson(new TextEncoder().encode(rawBody));
};
var postJsonWithConfidentialHttp = (runtime2, url, headers, body) => {
  const rawBody = runtime2.runInNodeMode((nodeRuntime, ...args) => {
    const requestUrl = args[0];
    const requestHeaders = args[1];
    const requestBody = JSON.stringify(args[2]);
    const confidentialHttpClient = new cre.capabilities.ConfidentialHTTPClient;
    const response = confidentialHttpClient.sendRequest(nodeRuntime, {
      request: {
        url: requestUrl,
    ...
    return new TextDecoder().decode(response.body);
  }, consensusIdenticalAggregation())(url, headers, body).result();
  return decodeJson(new TextEncoder().encode(rawBody));
};
var fetchOnchainAssetData = (runtime2, evmClient, registryAddress, assetId) => {
  const callDataCore = encodeFunctionData({
    abi: ASSET_REGISTRY_ABI,
    functionName: "getAssetCore",
    args: [assetId]
  });
  const resultCore = evmClient.callContract(runtime2, {
    call: encodeCallMsg({
      from: zeroAddress,
      to: registryAddress,
      data: callDataCore
  ...
  const resultMetadata = evmClient.callContract(runtime2, {
    call: encodeCallMsg({
      from: zeroAddress,
      to: registryAddress,
  ...
  return {
    ...
    metadataHash: decodedMetadata[0]
  };
};
var fetchNavSnapshot = (runtime2, evmClient, navOracleAddress, assetId) => {
  const isFreshCallData = encodeFunctionData({
    abi: NAV_ORACLE_ABI,
    functionName: "isFresh",
    args: [assetId]
  });
  const isFreshResult = evmClient.callContract(runtime2, {
    call: encodeCallMsg({
      from: zeroAddress,
      to: navOracleAddress,
  ...
  const navDataResult = evmClient.callContract(runtime2, {
    call: encodeCallMsg({
      from: zeroAddress,
      to: navOracleAddress,
...
var fetchRequestedBorrowAmount = (runtime2, evmClient, registryAddress, borrower, assetId) => {
...
var deterministicUnderwriting = (input, baseRateBps, rateSpreadPerRiskTier, maxLtvBaseBps, maxLtvReductionPerRiskTier) => {
  const dscr = input.assetMetrics.rentalCoverageRatio;
  const borrowerHealth = input.borrowerMetrics.creditRiskScore * 0.6 + (1 - input.borrowerMetrics.debtToIncome) * 0.4;
  let riskTier;
  if (dscr >= 1.5 && borrowerHealth >= 0.75) {
    riskTier = 1;
  } else if (dscr >= 1.25 && borrowerHealth >= 0.65) {
    riskTier = 2;
  } else if (dscr >= 1.1 && borrowerHealth >= 0.6) {
    riskTier = 3;
  } else if (dscr >= 1 || borrowerHealth >= 0.8) {
    riskTier = 4;
  } else {
    riskTier = 5;
  }
  ...
  maxLtvBps = clamp(maxLtvBps, 0, 1e4);
...
var hardDenyOutput = (reason, baseRateBps, rateSpreadPerRiskTier) => {
  return {
    approved: false,
    riskTier: 5,
    maxLtvBps: 0,
...
var runUnderwritingPolicy = (input, compliance, baseRateBps, rateSpreadPerRiskTier, maxLtvBaseBps, maxLtvReductionPerRiskTier) => {
  ...
  return deterministicUnderwriting(input, baseRateBps, rateSpreadPerRiskTier, maxLtvBaseBps, maxLtvReductionPerRiskTier);
};
var asObject = (value2) => {
  return typeof value2 === "object" && value2 !== null ? value2 : {};
};
var toStringArray = (value2) => {
  if (!Array.isArray(value2))
    return [];
...
var buildRiskFlags = (input, decision, hardDenyReason) => {
  const flags = [];
  if (hardDenyReason)
    flags.push(hardDenyReason);
  if (input.borrowerMetrics.debtToIncome > 0.5)
    flags.push("HIGH_DTI");
  if (input.assetMetrics.navVolatility > 0.1)
    flags.push("ELEVATED_VOLATILITY");
  if (input.macroContext.interestRateEnvironment === "HIGH")
    flags.push("MACRO_TIGHTENING");
  if (input.macroContext.propertyIndexTrend === "DECLINING")
    flags.push("DECLINING_PROPERTY_INDEX");
...
var generateAIExplanation = (runtime2, model, geminiApiKey, input, deterministicDecision, riskFlags, provenance) => {
  ...
  try {
    ...
    const confidenceLevel = confidenceRaw === "LOW" || confidenceRaw === "HIGH" ? confidenceRaw : "MEDIUM";
    return {
    ...
    logAudit(runtime2, "ai_explanation_fallback", {
      model,
      error: String(error)
    });
    return {
...
var computeAndHashNav = (assetId, assetValue, assetMetrics, macroContext) => {
  const trendAdjustmentBps = {
    RISING: 10300,
    STABLE: 1e4,
    DECLINING: 9500
  };
...
var encodeNavReport = (assetId, nav, sourceHash) => {
  return encodeAbiParameters([{ type: "uint256" }, { type: "uint256" }, { type: "bytes32" }], [assetId, nav, sourceHash]);
};
var updateNavOnchain = (runtime2, evmClient, navOracleAddress, assetId, nav, sourceHash) => {
  ...
  const writeResult = evmClient.writeReport(runtime2, {
    ...
    report: report2,
    gasConfig: runtime2.config.gasLimit ? { gasLimit: runtime2.config.gasLimit } : undefined
  }).result();
...
var onUnderwritingRequest = async (runtime2, eventLog) => {
  ...
  if (processedRequestIds.has(requestId)) {
    logStep(runtime2, requestId, "0.1", "Replay detected in runtime cache, skipping");
    logAudit(runtime2, "underwriting_replay_skipped", {
      requestId,
      borrower,
  ...
  logStep(runtime2, requestId, "1", "Event decoded", {
    borrower,
    assetId: assetId.toString(),
    txHash: txHashHex,
    logIndex: eventLog.index,
  ...
  if (requestedBorrowAmountFromRegistry === 0n) {
    logStep(runtime2, requestId, "3.1", "No pending requested amount in registry, skipping");
    logAudit(runtime2, "no_pending_request_skip", {
      requestId,
      borrower,
      assetId: assetId.toString(),
      intendedBorrowAmountFromEvent: intendedBorrowAmountFromEvent.toString()
  ...
  if (intendedBorrowAmountFromEvent > 0n && intendedBorrowAmountFromEvent !== requestedBorrowAmountFromRegistry) {
    logStep(runtime2, requestId, "3.2", "Event amount and registry amount mismatch, proceeding with registry amount", {
      intendedBorrowAmountFromEvent: intendedBorrowAmountFromEvent.toString(),
      requestedBorrowAmountFromRegistry: requestedBorrowAmountFromRegistry.toString()
    });
  }
  ...
  if (deniedStatuses.has(assetData.currentStatus)) {
    logAudit(runtime2, "asset_status_hard_deny", {
      requestId,
      borrower,
      assetId: assetId.toString(),
    ...
    logStep(runtime2, requestId, "5", "Safety gate triggered: denied asset status", {
      currentStatus: assetData.currentStatus
    });
  }
  if (!safetyGateReason && (!navSnapshot.isFresh || navSnapshot.nav === 0n)) {
    logStep(runtime2, requestId, "5", "NAV stale/missing; computing and publishing NAV update");
    const navComputation = computeAndHashNav(assetId, assetData.assetValue, assetPerformance, macroData);
    const navUpdateTxHash = updateNavOnchain(runtime2, evmClient, navOracleAddress, assetId, navComputation.nav, navComputation.sourceHash);
    logAudit(runtime2, "nav_updated_onchain", {
      requestId,
      borrower,
      assetId: assetId.toString(),
    ...
    navSnapshot = fetchNavSnapshot(runtime2, evmClient, navOracleAddress, assetId);
    logStep(runtime2, requestId, "5.1", "NAV refresh check after update", {
      isFresh: navSnapshot.isFresh,
      nav: navSnapshot.nav.toString(),
      navUpdatedAt: navSnapshot.updatedAt.toString()
    });
    if (!navSnapshot.isFresh || navSnapshot.nav === 0n) {
      logAudit(runtime2, "stale_or_missing_nav_deny", {
        requestId,
        borrower,
        assetId: assetId.toString(),
        isFresh: navSnapshot.isFresh,
      ...
      logStep(runtime2, requestId, "5.2", "Safety gate triggered: NAV still stale after update", {
        isFresh: navSnapshot.isFresh,
        nav: navSnapshot.nav.toString()
      });
    }
...
var onRiskMonitorCron = async (runtime2) => {
  logStep(runtime2, "CRON", "1", "Starting Risk Monitor Agent");
  const macroData = fetchMacroData();
  logStep(runtime2, "CRON", "2", "Macro data fetched", macroData);
  if (macroData.interestRateEnvironment === "HIGH" || macroData.propertyIndexTrend === "DECLINING") {
    logAudit(runtime2, "systemic_risk_detected", {
      reason: "Adverse macro environment",
      macroData
    });
    logStep(runtime2, "CRON", "3", "Systemic risk detected. Alert generated.");
  } else {
    logStep(runtime2, "CRON", "3", "Macro environment stable. No action required.");
  }
  return true;
};
var initWorkflow = (config) => {
  ...
  const cronTrigger = cron.trigger({ schedule: config.schedule ?? "0 0 * * *" });
  return [
...
async function main() {
  ...
  const runner = await Runner.newRunner({
    configParser: (config) => JSON.parse(new TextDecoder().decode(config))
  });
