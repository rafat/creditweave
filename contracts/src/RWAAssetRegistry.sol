...
contract RWAAssetRegistry is AccessControl, Pausable, ReentrancyGuard {
    ...
    mapping(uint256 => RealWorldAsset) internal assets;
        ...
        returns (bool triggered)
    {
        ...
        uint256 nextDueDate = _syncSchedule(asset, logicAddr);
        uint256 daysPastDue = _setDaysInDefault(asset, nextDueDate);
        ...
        _maybeIncrementMissedPayments(asset, newHealth, oldStatus);

        return _emitIfActionable(_assetId, newStatus, asset.missedPayments, daysPastDue);
    }

    function _syncSchedule(RealWorldAsset storage asset, address logicAddr)
        internal
        returns (uint256 nextDueDate)
    {
        (uint256 dueDate, uint256 expectedPeriodicPayment, uint256 maturityDate) =
            ICashFlowLogic(logicAddr).getSchedule();
        asset.nextPaymentDueDate = dueDate;
        asset.expectedMonthlyPayment = expectedPeriodicPayment;
        asset.expectedMaturityDate = maturityDate;
        return dueDate;
    }

    function _setDaysInDefault(RealWorldAsset storage asset, uint256 nextDueDate)
        internal
        returns (uint256 daysPastDue)
    {
    ...
    function _maybeIncrementMissedPayments(
        RealWorldAsset storage asset,
    ...
    function _emitIfActionable(
        ...
        if (newStatus == RWACommonTypes.AssetStatus.DEFAULTED) {
            emit DefaultTriggered(assetId, missedPayments, daysPastDue, block.timestamp);
            return true;
        }

        if (newStatus == RWACommonTypes.AssetStatus.LIQUIDATING) {
            emit LiquidationInitiated(assetId, daysPastDue, block.timestamp);
            return true;
        }
    ...
    /// @notice Small, frontend-friendly getter for core fields
    function getAssetCore(uint256 _assetId)
        external
        view
        returns (
            uint256 assetId,
    ...
    /// @notice Frontend-friendly getter for schedule fields
    function getAssetSchedule(uint256 _assetId)
        external
        view
        returns (
            uint256 nextPaymentDueDate,
        ...
        )
    {
        ...
        require(asset.assetId != 0, "Asset does not exist");
        return (
            asset.nextPaymentDueDate,
            asset.expectedMonthlyPayment,
            asset.expectedMaturityDate
        );
    }

    /// @notice Frontend-friendly getter for linked contracts
    function getAssetLinks(uint256 _assetId)
        external
        view
        returns (
            address logicContract,
